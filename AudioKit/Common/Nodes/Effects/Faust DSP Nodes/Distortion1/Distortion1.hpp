...
class Distortion1DSP : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("high_freq", &fVslider5, 5000.0f, 1000.0f, 12000.0f, 10.0f);
		ui_interface->declare(&fVslider6, "name", "low freq");
		ui_interface->declare(&fVslider6, "tooltip", "low-freq cutoff Hz");
		ui_interface->addVerticalSlider("low_freq", &fVslider6, 130.0f, 20.0f, 1000.0f, 10.0f);
		ui_interface->declare(&fCheckbox0, "name", "low highcutoff");
		ui_interface->addCheckButton("on_off", &fCheckbox0);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp1 = (fTemp0 + (fSlow6 * fRec3[1]));
