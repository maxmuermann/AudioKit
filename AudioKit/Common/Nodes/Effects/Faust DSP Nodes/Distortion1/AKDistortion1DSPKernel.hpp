...
public:
    ...
    void init(int _channels, double _sampleRate) override {
        ...
        classInitDistortion1DSP(_sampleRate);

        dsp = newDistortion1DSP();
        initDistortion1DSP(dsp, _sampleRate);
    ...
    void process(AUAudioFrameCount frameCount, AUAudioFrameCount bufferOffset) override {
        ...
        if (started) {
            computeDistortion1DSP(dsp, frameCount, tmpin, tmpout);
        } else {
            tmpout[0] = tmpin[0];
            tmpout[1] = tmpin[1];
        }
